public class Rectangle extends GeometricFigure {
    ...
    public String toString() {
        return  "<"+super.getShape()+" x=\""+super.getPosition().getX()+"\" y=\""+super.getPosition().getY()+"\" width=\""+super.getSize().getWidth()+"\" height=\""+super.getSize().getHeight()+"\" transform=\"translate(0, 0)\" fill=\""+this.getColor()+"\"/>\n" +
                "\t<text x=\""+((super.getPosition().getX()+super.getSize().getWidth()/2)+"\" y=\""+(super.getPosition().getY()+super.getSize().getHeight()/2))+"\" text-anchor=\"middle\" alignment-baseline=\"middle\" fill=\"white\" font-size=\"20\">"+this.getName()+"</text>\n";
    }
}
